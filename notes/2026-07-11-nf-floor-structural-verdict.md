...
- 🧵 **Fabric** — **STORE**: The note delivers a genuine, checkable result: it shows term-by-term that the chain's noise_figure_floor_dB and the quantum repo's F_dec_300K_dB are the *same* closed-form expression evaluated at different temperatures (353 K vs 300 K), not an independent coincidence, and backs this with a falsifiable prediction (§6) and a concrete task (add one export line to run_all.py). Every quoted number I can check against the supplied reference JSON matches exactly: noise_figure_floor_dB=2.768939660565078, F_dec_300K_dB=2.7749934346586986, M_th_300K/353K, cw_regen_gain_dB_at_0p7=9.66100611708918, pulse_gain_dB_at_0p7_streaming, and both cell-length figures (576.62 / 582.80 nm) used correctly in §7 to resolve the cascade-length question. Reworking the F=2-1/G step myself confirms the arithmetic is internally consistent (F_dB = 10log10(2-1/G_linear)), though the note's write-up glosses over this linear-to-dB step and briefly juxtaposes a dB-annotated G with the linear formula, which is a minor exposition flaw rather than a computational error. Labels (demonstrated/in-model/open) are applied with appropriate care, especially the separation of the arithmetic identity (demonstrated) from the physical interpretation of the Caves floor (in-model), and limitations are stated honestly (absolute calibration still owed to the Boltzmann-Maxwell tier; the falsifiable prediction is repo-state-dependent). This is durable, actionable WP1 material that changes how the 2.77 dB agreement should be described in future work.
  - The formula chain in Derivations A/B states 'F = 2 - 1/G' without making explicit that G there is the linear ratio while the quoted output is 10*log10(F_linear); the intermediate annotation 'G = +9.6610061 dB' momentarily conflates the dB-reported value with the linear quantity actually used in the F formula -- worth a clarifying edit.
  - The core 'structural, not coincidental' conclusion is stated in the note itself to already appear in the July manuscript revisions; the added value here is the derivation/prediction/cascade-resolution, which is real but should be framed explicitly as an extension rather than a first discovery to avoid double-counting novelty.
  - Section 5's low-temperature divergence table is asserted to ~10 digits of precision without showing the intermediate τ/Q arithmetic at 77 K and 4 K the way it did for 353/300 K, so a reader cannot fully re-derive those two rows from the note alone.
- 🌊 **Kinetic** — **STORE**: The note delivers a genuine, checkable corrective result: it disproves the discussion prompt's premise that the two 2.77 dB figures come from independent derivations, reduces both to one closed-form expression, and verifies bit-exact agreement against the actual results.json values given (2.768939660565078 and 2.7749934346586986 match exactly; M_th, Q, gain, and cell-length figures all check against the reference outputs). It states a falsifiable prediction (§6), correctly separates demonstrated arithmetic from in-model physics claims, discloses method limits (hand-transcription, no code execution) and scope limits (absolute calibration still owned by WP2/bench), and resolves a second question (cascade length) with an explicit sensitivity analysis rather than hand-waving. This is exactly the kind of provenance-tracing verification work the project needs and would change how a reader interprets the 'independent agreement' claim in the manuscripts. One real flaw: the symbolic derivation chain in §1–2 never states the final linear-to-dB conversion needed to get F_dB = 10·log10(2 − 1/G_linear) from G quoted in dB — a reader following the written formula literally (using G in dB directly in F = 2 − 1/G) will not reproduce the quoted digits, even though the actual number is correct when reverse-engineered. This is a documentation/precision gap that should be fixed before promotion, but does not invalidate the underlying verified result.
  - The stated formula chain 'G = ... dB' then 'F = 2 − 1/G' omits the necessary G_dB→G_linear conversion and the final F_linear→F_dB conversion; as written it is not literally reproducible by a reader, even though the quoted digit-exact values are correct once the implicit steps are supplied.
  - The cascade-length section (§7) leans on a sibling note (2026-07-11-streaming-gain-detuning-artifact.md) whose promotion status is not established here; if that note is not itself promoted/verified, the 'resolved' framing in §7 is partly borrowed credibility.
  - The claim 'reproduced to ~10⁻¹³ relative from that formula' for cascade_per_cell_dB is asserted but no intermediate arithmetic is shown in the note, unlike the careful step-by-step verification given for the noise-figure derivation.
- ⚛️ **Quanta** — **STORE**: The note delivers a genuine checkable result: it resolves a specific question (is the 2.77 dB NF agreement between the chain and quantum-model repos independent confirmation of the Caves floor, or structural reuse of the same formula?) with a term-by-term algebraic reduction showing the two derivations are literally the same closed form evaluated at two temperatures. Every quoted number (2.768939660565078, 2.7749934346586986, M_th_300K=0.125, M_th_353K=0.14708333..., G≈9.6610061/9.7683256 dB) matches the supplied reference JSONs exactly, and the derived sensitivity (0.107 dB gain gap compressing to 0.006 dB via 1/(2G-1)≈0.057) is internally consistent and numerically verified against the reference. Labels (demonstrated/in-model/open) are applied correctly and consistently, including the honest separation of 'arithmetic demonstrated' from 'physics in-model' in §5, and the limitations section explicitly disclaims that the verdict is about provenance, not absolute calibration. This is exactly the kind of anti-hype correction the project values (guarding against reading coincidental-looking agreement as independent quantum validation), it supplies a falsifiable, cheap-to-run prediction, and it resolves a loose end (cascade-length independence of the floor) relevant to WP5's error-budget work. It is durable and would change how someone assessing the NF floor's evidentiary weight proceeds.
  - Derivations were hand-transcribed and checked with standalone calculators rather than by executing the actual repos, so exact matches to the reference JSON are the only independent check of transcription fidelity
  - The note is dense and multi-part (premise correction, two derivations, reduction, verdict, table, prediction, cascade-length aside) -- borderline for a single note, though each part supports one coherent verdict
  - Relies on companion/other draft notes (2026-07-10 and a second 2026-07-11 draft) not verifiable from the excerpt alone
