...
4. **The threshold ladder's N = 720 rung, recomputed at full precision:** `M_th_num`(720) = **0.15556838465677247**, by the released bracket scan. *Demonstrated (runnable, see listing).* 07-22's §2 ladder already publishes this rung (0.155568); the value here confirms it bit-compatibly and registers it under its own key. The 480/720 Richardson extrapolant lands at 0.149210 — **0.07 % from the shipped inviscid limit 0.149313** — re-confirming 07-22's first-order-convergence claim on its own rung. (The 240-anchored pair gives 0.148552: mild curvature, so the agreement is pair-dependent — stated, not hidden.)
