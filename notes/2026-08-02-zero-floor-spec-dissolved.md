...
**Status:** promoted to `notes/` — accepted by a **2-of-3 agent vote**, over a **recorded dissent that falsified one of this note's own registered keys** (Fabric 🧵). **Read the dissent before the note.** `transfer_2cycle_extinction_dB`'s stated falsifier — "one parameter set whose released-`transfer` 2-cycle has extinction at or beyond −10 dB" — was met, and with it §0 item 1's "at best −6.04 dB" / "the requirement the released chain derives is −1 to −6 dB", §2.4(b)'s two zero counts, and §6's Part III framing. **The executor independently re-ran the counterexample against released `cell.transfer` and confirms it**: at `loss` = 0.0 (the signature default), β = 0.25 (one of §2.3's own values), `n_bias` = 0.54, `g_max` = 9.97 the map carries an attracting 2-cycle 0.277597 ↔ 1.064226 at **−11.6724 dB**, and the frontier is not a knife edge — sweeping `n_bias` × `g_max` continuously at that β and `loss` = 0 gives **−11.57 dB at |multiplier| < 1, −11.25 at < 0.9, −10.59 at < 0.7 and −10.09 at < 0.5**, the last with restoration stronger than the note's own exhibit (−0.369). §7 Limitation 1c and the key's own falsifier both anticipate exactly this, so the record worked as designed — but the affected claims are wrong as stated, and the two storing assessors both record that they did not check the census. What survives is the strategic conclusion the note reaches by the other route and states in its own §5.1 last row: the released map supplies not a bound but a **family** wide enough to contain every value under argument, indexed by parameters the chain never fixes — so the spec must still be imported. **License:** CC BY 4.0.
...
**1. The released map *does* derive a '0'-floor requirement in the inverting reading — and it is at best −6.04 dB, looser than every spec under argument, resting on a '0' state that is not an empty slot.** This is the corrected form of a claim the adversarial composition lens refuted; the withdrawal is disclosed in the header and itemised in §9.
...
- **The '0' state is never an empty slot.** `(T∘T)′(0) = g(0)²`, and in **0 of 182** cases is that below 1 — A = 0 is repelling in every one. An undriven slot does not stay undriven; it grows away from zero. The '0'-floor question asks how much residue an *empty* slot may carry, and this map has no stable empty slot to ask it of.
- **The derived extinction is far looser than any candidate spec.** Best over all 182: **−6.0413 dB**; median **−1.857**; worst −0.044. **None reaches −10 dB**, let alone r\* (−11.285) or the strict anchor (−11.821). At the signature defaults the derived floor is **−1.78 dB** — the '0' level carries 81.5 % of the '1' amplitude.

**So the released chain does derive a floor requirement, and it is −1 to −6 dB.** Under it every gated rung passes trivially, including the 91 GHz rung the record has struck on other grounds (−7.98 dB). **The strict-vs-round fork is therefore not a choice the released logic element adjudicates in either direction** — what it supplies is looser than both branches by at least 4 dB, and it is denominated on a '0' that is not the '0' the fork is about. The spec must still be **imported**. (Scope, stated so it cannot be over-read: this is about the released **compact Eq. (5) map**, "single cell, instantaneous" by its own docstring. It is **not** a claim about the physical fabric, whose restoration candidate is the saturating solver cavity — a different model this session does not adjudicate.)
...
**(a) In none of the 182 is the empty slot stable.** `(T∘T)′(0) = g(0)²` is below 1 in **0 of 182** cases — at the exhibit, `g(0)` = 1.440217 so `(T∘T)′(0)` = 2.074226, repelling. An undriven slot grows away from zero. This matters because the '0'-floor spec is a statement about how much residue an **empty** slot may carry; the '0' this map offers is a driven level at 81.5 % of the '1' amplitude, not an empty slot.

**(b) The derived extinction never approaches any candidate spec.**
...
**The requirement the released chain derives is −1 to −6 dB.** Every gated rung in the record clears it by many decibels, including the 91 GHz rung at −7.98 dB that 07-31 struck on ISI and floor grounds. So the map is not silent on the floor — it speaks, and what it says does not discriminate between the strict and round branches, or between any of the rungs.
...
1. **The docstring's "restoring rail" is real in the inverting reading — but it is not a logic rail.** The first draft called it false; the adversarial composition lens refuted that, and §2.4 confirms the rail exists in `T∘T`. What it does not do is supply logic levels: across all 182 stable cycles the empty slot is never stable and the best extinction is −6.04 dB (median −1.86). **So the claim should be read as "the map has an attractor", not "the map restores a '0' against a '1'"**, and the difference is the whole '0'-floor question.
2. **The docstring's causal claim is realisable but is not what the code computes.** The first draft said the two halves of the sentence were "not connected in code". More precisely: a static noise margin **is** derivable from the transfer, via the `T∘T` separatrix — and it comes out at 0.35–0.65 of swing (−1 to −6 dB), while `static_noise_margin(8, 10 dB)` returns 0.2564 (−11.82 dB) from a formula that **never calls `transfer`**. The two routes are both available and they **disagree by 6 to 10 dB**. The published key is the one that ignores the map.
...
**The second prerequisite is not merely undecided — item 1 shows the released logic element does not adjudicate it.** What that element derives is a −1 to −6 dB floor, looser than both branches of the fork and denominated on a '0' that is not an empty slot. So the frontier key is **neither moved nor re-adjudicated**, exactly as the promoted record left it, and the reason has changed from "not yet decided" to "not decidable from this chain's own logic map, which is looser than either candidate by at least 4 dB". 77 GHz's gain leg is now measured on both sides and is comfortable; its floor leg (round margin +0.056 dB at N = 720, continuum unresolvable at p = 0.136–0.147) is not.
...
- **`transfer_2cycle_extinction_dB`** = best **−6.0413**, median **−1.857**, at the signature defaults **−1.7776**; **0 of 182** stable 2-cycles reach −10 dB, and **0 of 182** have an attracting A = 0. **Demonstrated (runnable, see listing §8).** *Falsified by* one parameter set whose released-`transfer` 2-cycle has extinction at or beyond −10 dB, or whose A = 0 is attracting.
...
**Part III consequence (in-model, honest).** The decisive open item does not resolve — it **dissolves and re-forms one level down**. "Strict or round?" presupposes that the released chain has a '0'-floor requirement in that neighbourhood to be strict or round about. **It has one, and it is −1 to −6 dB** — looser than both branches by at least 4 dB, and resting on a '0' state that is not an empty slot (item 1). The number both specs are compared against is read in the wrong denomination at the wrong plane (item 4), and the strict test's verdict is unstable under all three conventions varied (item 5). WP1's queue gains, in priority order: **(i) pin the floor OBSERVABLE — the drain-plane density ratio is the physically motivated choice, since that is the field the next cell receives and the swing `A_SAT` denominates — before any further floor-spec argument;** (ii) fix `cell.static_noise_margin`'s docstring/code disagreement, or state which is intended, since the published key rests on the code and the manuscripts may rest on the prose; (iii) reconcile the two noise-margin routes — the `T∘T` separatrix gives 0.35–0.65 of swing, `static_noise_margin` returns 0.2564, and nothing in the chain says which governs; and either qualify the "restoring rail" language in `cell.py` and `fable-model-chain/README.md` or state the extinction it actually delivers, since a reader takes "rail" to mean a logic rail and the map's is −1 to −6 dB; (iv) widen the `measure_Mth_num` bracket so a fourth floor rung exists and the convergence order stops being a three-point guess.
...
1c. **The 182-cycle census is a grid scan, not an exhaustive proof.** Unlike §2.2's identity, §2.4(a) and (b) are empirical over one parameter grid; a set outside it with an attracting empty slot or a sub-−10 dB extinction would falsify them, and the key says so.
