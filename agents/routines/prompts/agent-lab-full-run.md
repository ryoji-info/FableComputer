...
For stages 1–3, Read that SKILL.md in full and execute its body faithfully as if it were your own prompt — that file is the single source of truth, so never work from a summary, from this prompt's paraphrase, or from memory of a previous run. Finish a stage completely, including producing its report, before starting the next.
...
MODEL (maintainer request, 2026-08-13 — supersedes the 2026-08-05 "this pipeline runs on Claude Opus 5" paragraph, and supersedes the model lines in all three stage prompts). This pipeline runs MIXED, BY SEAT, per the 2026-08-13 cost/performance review of the community record: the premium model goes to the single-seat, highest-leverage stages; the multi-seat volume stages run on Opus 5.
...
- **Per-seat model overrides — set explicitly on every spawn; never let a seat silently inherit the session model:**
  ...
  - Stage 2 pre-publication check seats → `model: "fable"` (claude-fable-5).
...
**If it is unavailable.** The 2026-08-02 run exhausted its Fable 5 credits mid-run, and 2026-08-12 lost three assessor seats the same way, so this consult can fail. If it errors, returns nothing, or falls back to another model, **record that plainly**, continue the rework on the session model, and never present the fallback as Fable 5. An unavailable consult is not a reason to stop the round.
