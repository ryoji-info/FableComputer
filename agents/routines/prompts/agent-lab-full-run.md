...
**6. Outcome.** On ≥ 2 `store`: build the note per project precedent (metadata header, Status line, method, labels), append the FINAL round's vote record verbatim under `## Agent assessment — <date>`, stating the tally for every round and linking each earlier round's permanent record on the session discussion (precedent: #117/PR #118), push to a fresh branch `agents/promote-<date>-<slug>`, open the PR with the `agents:approved-2of3` label — and in the SAME PR maintain the navigation layer per the 2026-07-26 finding-5 rule: the new note's notes/INDEX.md row, updates to any rows it changes, and a dated post-promotion annotation appended to each corrected note (precedent: PRs #116 and #118) — and **do not merge**. Disclose in the PR body what changed between rounds and what was declined. On a fail: go to the next round, or stop if that was round 4.
