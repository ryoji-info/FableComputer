...
**Part I** designs the smallest complete machine on this fabric — a **half adder**: five regenerative cells computing Sum and Carry, one addition per 4-picosecond slot (2.5×10¹¹ additions/s, wave-pipelined) as the **design baseline**, at roughly 0.4 femtojoules per addition on the fabric side, in a footprint of about 12 × 16 µm. A five-pass feasibility chain converges on one self-consistent operating point and checks it end to end: gain budget, logic transfer, noise, disorder-limited yield, thermal ceiling, and the validity of the underlying hydrodynamics. **Since 2026-07-23 the community record qualifies that baseline:** with per-slot flushing required against cavity ring-down, fan-out-2 logic does not close at a 4-ps slot at any de-Q rate, and the defensible in-model operating point for F = 2 logic is **≈0.1 THz** — see [notes/2026-07-23-reset-switch-adjudication.md](notes/2026-07-23-reset-switch-adjudication.md), [notes/2026-07-22-cavity-ringdown-isi.md](notes/2026-07-22-cavity-ringdown-isi.md) and the burst-length proviso in [notes/2026-07-25-e1-burst-scaling-proviso.md](notes/2026-07-25-e1-burst-scaling-proviso.md).

**Part II** asks what the same fabric computes when its pulses are treated as what they are: quantum fields. A logic pulse turns out to be a *mesoscopic* object — the 1-dB compression knee sits at ≈38 plasmons **referred to the launch/drive plane** (input-referred band 22–51 across the CW and pulse observables), and the logic rail at ≈3,800 — a figure **anchored at neither plane** — so quantum noise is a design input, not a curiosity. Plane conventions per [notes/2026-07-17-what-the-38-quanta-knee-denominates-after-the-july-plane-aud.md](notes/2026-07-17-what-the-38-quanta-knee-denominates-after-the-july-plane-aud.md). Two results follow. An honest no-go: this cell cannot host plasmonic qubits (the single-plasmon nonlinearity misses by five orders of magnitude). And a constructive result: below the knee the fabric is a **quantum-limited analog tensor engine** — coherent pulses interfere at gate-programmed junctions and the result is digitized on-chip by the same threshold cells, re-programmed as a 1–2-bit flash decoder. Part II computes its error rates versus temperature and locates the quantum–classical crossover at T_Q ≈ 48 K.
...
Every headline number on this page is quoted at one point unless it says otherwise:
**T = 353 K** (the 80 °C cap; 300 K where a row says so), carrier **1 THz**, bias
**M/M_th,num = 0.7**, fan-out **2**, junction class **−1 dB**, and the **0.25 THz**
symbol rate as the *design baseline*. Two qualifications travel with it:

- **Timing.** In-model, once per-slot flushing is required against cavity ring-down,
  fan-out-2 logic closes at **≈0.1 THz**, not at the 4-ps slot — see
  [notes/2026-07-23-reset-switch-adjudication.md](notes/2026-07-23-reset-switch-adjudication.md).
- **Reference plane.** Knee and rail amplitudes are launch/drive-plane quantities; the
  rail is anchored at neither plane — see
  [notes/2026-07-17-what-the-38-quanta-knee-denominates-after-the-july-plane-aud.md](notes/2026-07-17-what-the-38-quanta-knee-denominates-after-the-july-plane-aud.md).

Cold-class figures (Part II's per-shot 2-bit and averaged modes) are an explicit fork
from the no-cryogenics charter and are labelled as such wherever they appear.
