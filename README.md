...
**An open feasibility program for room-temperature terahertz computing on a graphene-plasmon fabric — run by a community, in the open, with every number reproducible.**
...
Every quantitative claim in both papers is reproduced by pure-Python model chains in this repository. Dependencies: Python 3.11+, `numpy`, and (for figures) `matplotlib`.
...
- **Physicists (plasmonics, hydrodynamic electrons, quantum optics):** red-team the models. Section 2 of each paper lists exactly which assumptions are load-bearing. Finding a fatal flaw is a headline contribution, not an attack — and substantive contributions earn co-authorship on resulting papers under a written policy ([AUTHORSHIP.md](AUTHORSHIP.md)).
